...
export default function Programs({ onBookTrial }) {
  ...
  return (
    <div className="relative isolate pt-24 min-h-screen bg-obsidian">
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {PROGRAMS_DATA.map((program, i) => (
            <div
              key={program.id}
              className="bg-[#0A1D31] border border-white/5 hover:border-[#8DB6D7]/30 overflow-hidden scroll-fade card-glow"
            ...
            >
              <div className="aspect-[21/9] overflow-hidden">
                <img
                  ...
                  onError={(e) => { e.target.style.display = "none"; e.target.parentElement.style.background = "#0E2740"; }}
                />
              </div>
              <div className="p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-2 flex-shrink-0 ml-4">
                    <span className="text-xs bg-[#8DB6D7]/10 border border-[#8DB6D7]/30 text-[#8DB6D7] px-2 py-1 font-bold uppercase tracking-wider">
                      {program.level}
                    </span>
                  </div>
                </div>
                <p className="text-[#86A2BC] text-xs uppercase tracking-wider mb-3 font-bold">Age: {program.ageGroup}</p>
                <p className="text-[#92ABC4] text-sm leading-relaxed mb-5">{program.description}</p>

                <div className="mb-5">
                  <p className="text-xs font-bold uppercase tracking-widest text-[#86A2BC] mb-3">Key Benefits</p>
                  <div className="grid grid-cols-2 gap-2">
                    {program.benefits.map((b, j) => (
                      <div key={j} className="flex items-center gap-2 text-sm text-[#92ABC4]">
                        <Check className="w-3 h-3 text-[#8DB6D7] flex-shrink-0" /> {b}
                      </div>
                    ))}
                  </div>
                </div>

                <div className="bg-[#0E2740] border border-white/5 p-4 mb-5">
                  <p className="text-xs font-bold uppercase tracking-widest text-[#86A2BC] mb-1">Expected Outcome</p>
      ...
      {/* CTA Banner */}
      <div className="relative overflow-hidden bg-[#A9C9E3] py-20 px-6">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-60" aria-hidden="true">
          <DotGrid dotSize={3} gap={30} baseColor="#8DB6D7" activeColor="#FFFFFF" proximity={150} />
        </div>
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <button onClick={onBookTrial} className="bg-white hover:bg-[#E9F1F8] text-[#8DB6D7] font-bold uppercase tracking-widest px-8 py-4 text-sm inline-flex items-center gap-2 transition-colors duration-200">
