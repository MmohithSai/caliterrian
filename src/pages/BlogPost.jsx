...
function renderContent(content) {
  ...
  return content.split("\n").map((line, i) => {
    ...
    if (line.startsWith("- ")) return <li key={i} className="text-[#9AA7B6] text-sm leading-relaxed mb-2 ml-4">{line.slice(2)}</li>;
    if (line.match(/^\d\. /)) return <li key={i} className="text-[#9AA7B6] text-sm leading-relaxed mb-2 ml-4">{line.slice(3)}</li>;
    if (line.trim() === "") return <br key={i} />;
    const bold = line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-semibold">$1</strong>');
    return <p key={i} className="text-[#9AA7B6] text-base leading-relaxed mb-3" dangerouslySetInnerHTML={{ __html: bold }} />;
  });
}
