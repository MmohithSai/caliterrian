...
function renderContent(content) {
  ...
  return content.split("\n").map((line, i) => {
    ...
    if (line.startsWith("### ")) return <h3 key={i} className="font-heading text-2xl text-[#8DB6D7] tracking-wide mt-6 mb-3">{line.slice(4)}</h3>;
    if (line.startsWith("- ")) return <li key={i} className="text-[#92ABC4] text-sm leading-relaxed mb-2 ml-4">{line.slice(2)}</li>;
    if (line.match(/^\d\. /)) return <li key={i} className="text-[#92ABC4] text-sm leading-relaxed mb-2 ml-4">{line.slice(3)}</li>;
    if (line.trim() === "") return <br key={i} />;
    const bold = line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-semibold">$1</strong>');
    return <p key={i} className="text-[#92ABC4] text-base leading-relaxed mb-3" dangerouslySetInnerHTML={{ __html: bold }} />;
  });
}
  ...
  if (!post) {
    return (
      <div className="pt-24 min-h-screen bg-obsidian flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="text-[#92ABC4] mb-8">This article doesn't exist or may have been removed.</p>
  ...
  return (
    <div className="relative isolate pt-24 min-h-screen bg-obsidian">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-8">
        <Link to="/blog" className="flex items-center gap-2 text-[#92ABC4] hover:text-white text-sm transition-colors duration-200 mb-8">
      ...
      <article className="max-w-4xl mx-auto px-6 pb-20">
        <div className="mb-6">
          ...
          <p className="text-[#C9DCEC] text-lg leading-relaxed mb-8 max-w-[65ch]">{post.excerpt}</p>
          <div className="section-divider mb-8" />
        </div>
        <div className="prose-dark">{renderContent(post.content)}</div>
        <div className="mt-16 bg-[#0A1D31] border border-[#8DB6D7]/30 p-8">
          <h3 className="font-heading text-3xl text-white mb-3">READY TO START TRAINING?</h3>
          <p className="text-[#92ABC4] text-sm mb-6">Book a FREE trial session at Cali Terrain, Secunderabad.</p>
