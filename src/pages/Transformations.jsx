...
export default function Transformations({ onBookTrial }) {
  ...
  return (
    <div className="relative isolate pt-24 min-h-screen bg-obsidian">
      ...
      <div className="bg-[#A9C9E3] py-12 px-6">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12 pb-4">
        <div className="flex flex-wrap gap-2">
          {TYPES.map((type) => (
            <button key={type} onClick={() => setFilter(type)} className={`text-xs font-bold uppercase tracking-widest px-4 py-2 border transition-colors duration-200 ${filter === type ? "bg-[#8DB6D7] border-[#8DB6D7] text-[#041C38] shadow-[0_0_18px_rgba(141,182,215,0.35)]" : "bg-transparent border-white/20 text-[#92ABC4] hover:border-[#8DB6D7]/60 hover:text-white"}`}>{type}</button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        {filtered.length === 0 ? (
          <div className="text-center py-20"><p className="text-[#86A2BC] text-sm">No transformations found for this category.</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((item, i) => (
              <div key={item.id} className="bg-[#0A1D31] border border-white/5 hover:border-[#8DB6D7]/30 overflow-hidden scroll-fade card-glow" style={{ transitionDelay: `${(i % 3) * 0.1}s` }}>
                <div className="aspect-[16/9] bg-[#0E2740] border-b border-white/5 flex items-center justify-center">
                  <Trophy className="w-12 h-12 text-[#8DB6D7]/30" />
                </div>
                <div className="p-6">
                  <span className="inline-block bg-[#8DB6D7]/10 border border-[#8DB6D7]/30 text-[#8DB6D7] text-xs font-bold uppercase tracking-wider px-3 py-1 mb-4">{item.achievement_type}</span>
                  <h3 className="font-heading text-2xl text-white tracking-wide mb-2">{item.member_name}</h3>
                  <p className="text-[#C9DCEC] font-semibold text-sm mb-4">{item.summary}</p>
                  {item.testimonial && <p className="text-[#86A2BC] text-sm italic leading-relaxed mb-4"><span aria-hidden="true" className="font-heading text-xl text-[#8DB6D7] mr-1.5 align-[-0.12em]">"</span>{item.testimonial}"</p>}
                  {item.date && <p className="text-[#5A7896] text-xs uppercase tracking-wider">{item.date}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="relative overflow-hidden bg-[#081727] border-y border-white/5 py-20 px-6">
        ...
        <div className="relative max-w-4xl mx-auto text-center scroll-fade">
          ...
          <p className="text-[#92ABC4] text-base max-w-lg mx-auto mb-8">Every transformation started with a single decision. Book your free trial.</p>
