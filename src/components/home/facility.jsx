...
export function FacilitySection() {
  ...
  return (
    <Section id="facility">
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={vpOnce} variants={reveal} className="relative overflow-hidden rounded-sm border border-[#16324E]">
        <MediaSlot media={FACILITY.panorama} img={FACILITY.panorama.img} parallax scrim="ct-media__scrim--pano">
          {zones.slice(0, PIN_POS.length).map((zone, i) => (
            ...
            >
              <span className="flex h-8 w-8 items-center justify-center rounded-full border bg-[#051220]/80 backdrop-blur transition-transform">
        ...
        <div className="pointer-events-none absolute right-3 top-3 z-20 hidden w-60 border border-[#8DB6D7]/40 bg-[#051220]/90 p-3 backdrop-blur sm:block">
          <div className="overflow-hidden rounded-sm">
            <MediaSlot media={{ ...activeZone.media, ratio: "16/9" }} img={activeZone.media.img} align="" showLabel={false} />
          </div>
          <div className="mt-3 flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-sm border border-[#8DB6D7]/40 text-[#8DB6D7]">
              <ActiveIcon className="h-3.5 w-3.5" />
            </span>
            <h3 className="font-heading text-base tracking-wide text-white">{activeZone.name}</h3>
          </div>
          <p className="mt-2 text-[11px] leading-snug text-[#92ABC4]">{activeZone.desc}</p>
        </div>
      </motion.div>

      {/* Hotspot detail card (mobile — shown in-flow below the panorama since taps have no hover overlay to rely on) */}
      <div className="mt-3 border border-[#8DB6D7]/40 bg-[#051220]/90 p-3 sm:hidden">
...
function GalleryPlaceholder() {
  return (
    <div className="flex h-[78vh] min-h-[540px] w-full items-center justify-center rounded-lg border border-[#16324E] bg-[#030B14] sm:h-[86vh]">
