...
export default function TrialBookingModal({ open, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="relative bg-[#081727] border border-white/10 p-6 sm:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <button onClick={handleClose} aria-label="Close" className="absolute top-4 right-4 text-[#86A2BC] hover:text-white">
          <X className="w-5 h-5" />
        </button>
        ...
        {submitted ? (
          ...
          <div className="text-center py-6">
            <div className="mx-auto w-16 h-16 rounded-full bg-[#8DB6D7]/15 flex items-center justify-center mb-5">
              <CheckCircle2 className="w-9 h-9 text-[#8DB6D7]" />
            </div>
            <h2 className="font-heading text-3xl text-white mb-2">YOU'RE BOOKED!</h2>
            <p className="text-[#92ABC4] text-sm max-w-sm mx-auto mb-1">
              Thanks{form.name ? `, ${form.name.split(" ")[0]}` : ""} — we've received your free trial request.
            </p>
            <p className="text-[#86A2BC] text-sm max-w-sm mx-auto mb-7">
            ...
            <button
              onClick={handleClose}
              className="mt-3 text-sm text-[#86A2BC] hover:text-white transition-colors"
            >
              Done
            </button>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#0E2740] border border-white/10 focus:border-[#8DB6D7] text-white px-4 py-3 text-sm outline-none placeholder-[#5A7896] transition-colors duration-200"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#0E2740] border border-white/10 focus:border-[#8DB6D7] text-white px-4 py-3 text-sm outline-none transition-colors duration-200"
                >
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-[#86A2BC] mb-1.5 block">Preferred Time</label>
                <select
