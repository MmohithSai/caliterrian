    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[#5A7896] text-sm mt-1">{count} {count === 1 ? "record" : "records"}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          <Search className="w-4 h-4 text-[#5A7896] absolute left-3 top-1/2 -translate-y-1/2" />
          <input
            value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search name or phone..."
            className="w-full bg-[#0B2036] border border-white/10 focus:border-[#8DB6D7] text-white pl-9 pr-4 py-2.5 text-sm outline-none rounded-sm placeholder-[#40607F]"
          />
        </div>
        <select
          value={status} onChange={(e) => setStatus(e.target.value)}
          className="bg-[#0B2036] border border-white/10 focus:border-[#8DB6D7] text-white px-4 py-2.5 text-sm outline-none rounded-sm cursor-pointer"
        >
      ...
      {loading && isFirstLoad ? (
        <div className="flex items-center justify-center py-20 text-[#5A7896]">
          <Loader2 className="w-5 h-5 animate-spin mr-2" /> Loading...
        </div>
      ) : !error && rows.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-[#5A7896]">
        ...
        <>
          ...
          <div className="hidden md:block border border-white/10 rounded-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-[#0B2036] text-[#5A7896] text-xs uppercase tracking-wider">
                <tr>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row.id} onClick={() => setSelected(row)}
                    ...
                    {columns.map((c) => (
                      <td key={c.label} className="px-4 py-3 text-[#D9E6F2]">{c.render ? c.render(row) : (row[c.key] || "—")}</td>
                    ))}
                    ...
                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-2">
                        <a href={telHref(row.phone)} title="Call" className="p-1.5 text-[#8DB6D7] hover:bg-[#8DB6D7]/10 rounded-sm"><Phone className="w-4 h-4" /></a>
          ...
          <div className="md:hidden space-y-3">
            {rows.map((row) => (
              <div key={row.id} onClick={() => setSelected(row)}
                className="border border-white/10 bg-[#0A1D31] rounded-sm p-4 active:bg-white/[0.03]">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="min-w-0">
                    <p className="text-white font-bold truncate">{row.name}</p>
                    <p className="text-[#5A7896] text-xs">{row.phone} · {fmtDate(row.created_at)}</p>
                  </div>
                  <StatusBadge value={row.status} options={statuses} />
                </div>
                {columns.filter((c) => c.mobile).map((c) => (
                  <p key={c.label} className="text-[#92ABC4] text-xs">{c.label}: <span className="text-[#D9E6F2]">{c.render ? c.render(row) : (row[c.key] || "—")}</span></p>
                ))}
                <div className="flex gap-2 mt-3" onClick={(e) => e.stopPropagation()}>
                  <a href={telHref(row.phone)} className="flex-1 flex items-center justify-center gap-1.5 bg-[#8DB6D7]/15 text-[#8DB6D7] text-sm font-semibold py-2 rounded-sm"><Phone className="w-4 h-4" /> Call</a>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-5 text-sm">
              <button disabled={page === 0} onClick={() => setPage((p) => p - 1)}
                className="flex items-center gap-1 text-[#92ABC4] hover:text-white disabled:opacity-30 disabled:cursor-not-allowed">
                <ChevronLeft className="w-4 h-4" /> Prev
              </button>
              <span className="text-[#5A7896]">Page {page + 1} of {totalPages}</span>
