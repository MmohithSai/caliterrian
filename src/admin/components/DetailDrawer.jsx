...
export default function DetailDrawer({ record, fields, statuses, onClose, onStatusChange, onSaveNotes, onArchive }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex justify-end" role="dialog" aria-modal="true" aria-label="Record details">
      ...
      <div className="relative w-full max-w-md h-full bg-[#051220] border-l border-white/10 overflow-y-auto">
        <div className="sticky top-0 bg-[#051220] border-b border-white/10 px-5 py-4 flex items-center justify-between">
          <div>
            <h2 className="font-heading text-xl text-white leading-tight">{record.name}</h2>
            <p className="text-[#5A7896] text-xs mt-0.5">Added {fmtDateTime(record.created_at)}</p>
          </div>
          <button onClick={onClose} aria-label="Close" className="text-[#5A7896] hover:text-white p-1">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <a href={telHref(record.phone)} className="flex items-center justify-center gap-2 bg-[#8DB6D7] text-[#041C38] font-bold text-sm py-2.5 rounded-sm">
          ...
          {/* Status */}
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-[#5A7896] mb-1.5">Status</p>
          ...
          <dl className="space-y-3">
            {fields.map((f) => {
              ...
              return (
                <div key={f.label}>
                  <dt className="text-xs font-bold uppercase tracking-widest text-[#5A7896] mb-0.5">{f.label}</dt>
                  <dd className="text-sm text-[#D9E6F2] whitespace-pre-wrap break-words">{val}</dd>
                </div>
              );
            })}
          </dl>
          ...
          {/* Notes */}
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-[#5A7896] mb-1.5">Private Notes</p>
            <textarea
              value={notes} onChange={(e) => setNotes(e.target.value)} rows={4}
              placeholder="Follow-up notes (only you can see these)..."
              className="w-full bg-[#0E2740] border border-white/10 focus:border-[#8DB6D7] text-white px-3 py-2 text-sm outline-none resize-none rounded-sm"
            />
            <button onClick={saveNotes} disabled={savingNotes} className="mt-2 flex items-center gap-2 text-sm font-semibold text-[#8DB6D7] hover:text-white disabled:opacity-50">
              <Save className="w-4 h-4" /> {savingNotes ? "Saving..." : "Save notes"}
            </button>
          </div>

          <button onClick={archive} className="flex items-center gap-2 text-sm text-[#5A7896] hover:text-red-400 pt-2">
