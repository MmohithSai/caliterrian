...
function StatCard({ icon: Icon, label, value, to, accent = "#8DB6D7", loading }) {
  const body = (
    <div className="border border-white/10 bg-[#0A1D31] rounded-sm p-5 h-full hover:border-white/20 transition-colors">
      <div className="flex items-center justify-between mb-3">
        ...
        {to && <ArrowRight className="w-4 h-4 text-[#40607F]" />}
      </div>
      <p className="text-3xl font-heading text-white">{loading ? <Loader2 className="w-5 h-5 animate-spin" /> : value}</p>
      <p className="text-[#5A7896] text-xs uppercase tracking-wider mt-1">{label}</p>
    </div>
  );
...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      <p className="text-[#5A7896] text-sm mb-6">Operational overview — leads & trial bookings.</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard icon={Clock} label="Pending bookings" value={stats?.pendingBookings ?? 0} to="/admin/bookings?status=pending" accent="#8DB6D7" loading={loading} />
      ...
      {/* Recent activity */}
      <div className="border border-white/10 bg-[#0A1D31] rounded-sm">
        <div className="px-5 py-4 border-b border-white/10">
          <h2 className="font-heading text-lg text-white">Recent activity</h2>
        </div>
        {loading ? (
          <div className="flex items-center justify-center py-12 text-[#5A7896]"><Loader2 className="w-5 h-5 animate-spin" /></div>
        ) : activity.length === 0 ? (
          <p className="text-[#5A7896] text-sm px-5 py-10 text-center">No activity yet. Submissions will appear here.</p>
        ) : (
                ...
                <Link to={item.kind === "lead" ? "/admin/leads" : "/admin/bookings"}
                  ...
                  <div className="min-w-0">
                    <p className="text-white text-sm font-medium truncate">
                      {item.name} <span className="text-[#40607F] font-normal">· {item.kind === "lead" ? "Lead" : "Booking"}</span>
                    </p>
                    <p className="text-[#5A7896] text-xs">{item.phone} · {fmtDateTime(item.created_at)}</p>
                  </div>
